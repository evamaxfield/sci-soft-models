...
EXAMPLE_AWARDS = [
    soft_search.AwardDetails(
        id_="2211275",
        title="Collaborative Research: EAGER: Characterizing Research Software from NSF Awards",
    ...
    soft_search.AwardDetails(
        ...
        title="Collaborative Research: EAGER: Characterizing Research Software from NSF Awards",
        abstract=EAGER_AWARD_ABSTRACT,
        outcomes_report="We did not produce any software. This is meerly a test to see how the prediction changes. It shouldn't change. The prediction should be in favor of producing software.",
    ),
    soft_search.AwardDetails(
        id_="2211275",
        title="Collaborative Research: EAGER: Characterizing Research Software from NSF Awards",
        abstract=EAGER_AWARD_ABSTRACT,
        outcomes_report="Ultimately we didn't produce any software or code as a part of our research as a majority of the work was spent on surveys and interviews with research software developers.",
    ),
]
...
EXAMPLE_EXPECTED_OUTPUTS = [
    soft_search.AwardDetailsWithSoftwareProductionPrediction(
        ...
        software_production_prediction="software",
        confidence=0.9,
    ),
    soft_search.AwardDetailsWithSoftwareProductionPrediction(
    ...
    soft_search.AwardDetailsWithSoftwareProductionPrediction(
        ...
        software_production_prediction="no_software",
        confidence=0.9,
    ),
]
...
def test_nsf_soft_search_v2() -> None:
    ...
    software_predictions = soft_search.predict_software_production_from_awards(EXAMPLE_AWARDS)
    # Assert all matches
    for predicted_output, expected_output in zip(
        software_predictions,
        EXAMPLE_EXPECTED_OUTPUTS,
        strict=True,
    ):
        assert predicted_output.software_production_prediction == expected_output.software_production_prediction